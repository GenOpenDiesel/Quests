...
| `reverse-if-broken`       | Whether breaking a block whose placement credited this task should reverse that progress. | Boolean                        | No       | true    | The player, world, material and exact coordinates must match. Breaking an unrelated block of the same material does not remove progress.                                                                                                                                         |
| `allow-negative-progress` | Compatibility option retained for existing quest files.           | Boolean                        | No       | false   | Reversal never lowers building progress below zero.                                                                                                                                                                                                                             |
| `worlds`                  | Worlds which should count towards the progress.                   | List of world names            | No       | \-      | \-                                                                                                                                                                                                                                                                              |

By default, the fork remembers placements which actually increased each building task. If the
same player breaks that exact block before the placement credit expires, one point is reversed.
Breaking natural blocks or blocks at different coordinates has no effect. Placement credits use
`options.antifarm-place-break-reset-seconds` (10 minutes by default), which keeps ordinary building
corrections neutral without retaining large numbers of block locations indefinitely. Set
`reverse-if-broken: false` on a task to count raw placements without reversal.
