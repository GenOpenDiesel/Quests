...
| `reverse-if-broken`       | Compatibility option. This fork always decrements progress when a matching block is broken to prevent place/break farming. | Boolean                        | No       | true    | The protection is enforced even when this option is set to `false`, but progress never falls below zero.                                                                                                                                                                        |
| `allow-negative-progress` | Compatibility option retained for existing quest files.           | Boolean                        | No       | false   | Anti-farm reversal never lowers building progress below zero.                                                                                                                                                                                                                   |
