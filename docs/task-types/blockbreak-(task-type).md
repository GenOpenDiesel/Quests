...
| `reverse-if-placed`        | Compatibility option. This fork always decrements progress when a matching block is placed to prevent place/break farming. | Boolean                        | No       | true    | The protection is enforced even when this option is set to `false`. Progress may temporarily become negative so the same block cannot add progress when broken.                                                                                                                  |
...
| `allow-negative-progress`  | Compatibility option. Anti-farm placement debt may enter the negatives.                        | Boolean                        | No       | true    | The anti-farm protection takes priority over this option.                                                                                                                                                                                                                       |
